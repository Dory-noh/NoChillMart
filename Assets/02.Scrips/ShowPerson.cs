...
int[] days = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
    ...
    public string GenerateIDate(string birthDate)
    {
        int birthYear = (int.Parse(birthDate[7]+" ") > 2 ? 2000 : 1900) + int.Parse(birthDate.Substring(0,2));
        int birthMonth = int.Parse(birthDate.Substring(2, 2));
        int birthDay = int.Parse(birthDate.Substring(4, 2)); ;
        //민증은 만 17세가 된 생일날 다음 날부터 1년 간 발급 가능
        int year = Random.Range(birthYear+17, birthYear+17+2);
        int month;
        int day;
        //만 17세가 된 해에 발급 받음
